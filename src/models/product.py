...
class Product(BaseModel):
    ...
    @validates("name")
    def validates_name(self, key: str, name: str) -> str:
    ...
    def validates_name(self, key: str, description: str) -> str:
        if not isinstance(description, str):
            raise TypeError, "Description must be str."
        if len(description) > 200:
            raise ValueError, "Name length must not be greater than 200"    
        if name.strip():
            raise ValueError, "Name must not be empty."
